...
rho   = 1.225;  Vinf  = 10;  alpha = 4.0*pi/180;
...
L  = sum (dP .* (-nvers(2,:)'*cos(alpha)+nvers(1,:)'*sin(alpha)) .* len'); % lift
L  = sum (dP .* (-nvers(2,:)' ) .* len') ; % Fy
cL = L ./ (0.5 * rho * Vinf^2 * c);

D  = sum (dP .* (-nvers(1,:)'*cos(alpha)-nvers(2,:)'*sin(alpha)) .* len');
D  = sum (dP .* (-nvers(1,:)' ) .* len') ; % Fx
...
% +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
xb0 = -3.1 ; xb1 = 3.4 ; nx = 250 ; 
yb0 = -3.1 ; yb1 = 3.1 ; ny = 250 ;
xbox = [ xb0 , xb1 ] ;
ybox = [ yb0 , yb1 ] ;
nbox = [ nx  , ny  ] ;
...
m_1 = mod([np-1:2*np-2],np)+1  ;
m1  = mod([   1:  np  ],np)+1  ;
ns_c = [ -( y_c(m1) - y_c ) , ( x_c(m1) - x_c ) ] ;
% ns_c = [ -( y_c(2:end) - y_c(1:end-1) ) , ( x_c(2:end) - x_c(1:end-1) ) ] ;

% No pressure terms : L = - int_S{ rho u u.n }
dR_app = - 0.5*rho*(uu_c(m1,:)'.*(ones(2,1)*sum(uu_c(m1,:)'.*ns_c'))+ ...
                    uu_c(: ,:)'.*(ones(2,1)*sum(uu_c(: ,:)'.*ns_c'))) ;

R_app = sum(dR_app,2) ;

% With pressure terms : L = - int_S{ rho u u.n } - int_S{Pn}
dR = -0.5 * rho*(uu_c(m1,:)'.*(ones(2,1)*sum(uu_c(m1,:)'.* ns_c'))+ ...
                 uu_c(: ,:)'.*(ones(2,1)*sum(uu_c(: ,:)'.* ns_c'))) ...
    - 0.5 *( ones(2,1)*p_c(m1,:)' + ones(2,1)*p_c(:)' ) .* ns_c' ;
...
dsigUy = 0.05 ;
dsigsD = 0.5^2 * rho.^2 * ( ...
        ( - sum ( uu_c'.*ns_c'        ) - uu_c(:  ,1)'.*ns_c(:  ,1)'   ...
          - sum ( uu_c'.*ns_c(m_1,:)' ) - uu_c(m_1,1)'.*ns_c(m_1,1)' + ...
  i_sigP * ( uu_c(:  ,1)'.*ns_c(:  ,1)' + uu_c(m_1,1)'.*ns_c(m_1,1)' ) ) .^2 .* dsigUx .^ 2 + ...
        (                               - uu_c(:  ,1)'.*ns_c(:  ,2)'   ...
                                        - uu_c(m_1,1)'.*ns_c(m_1,2)' + ...
  i_sigP * ( uu_c(:  ,2)'.*ns_c(:  ,1)' + uu_c(m_1,2)'.*ns_c(m_1,1)' ) ) .^2 .* dsigUy .^ 2 ...     
        ) ;
...
dsigsL = 0.5^2 * rho.^2 * ( ...
        ( - sum ( uu_c'.*ns_c'        ) - uu_c(:  ,2)'.*ns_c(:  ,2)'   ...
          - sum ( uu_c'.*ns_c(m_1,:)' ) - uu_c(m_1,2)'.*ns_c(m_1,2)' + ...
  i_sigP * ( uu_c(:  ,2)'.*ns_c(:  ,2)' + uu_c(m_1,2)'.*ns_c(m_1,2)' ) ) .^2 .* dsigUy .^ 2 + ...
        (                               - uu_c(:  ,2)'.*ns_c(:  ,1)'   ...
                                        - uu_c(m_1,2)'.*ns_c(m_1,1)' + ...
  i_sigP * ( uu_c(:  ,1)'.*ns_c(:  ,2)' + uu_c(m_1,1)'.*ns_c(m_1,2)' ) ) .^2 .* dsigUx .^ 2 ...   
        ) ;
...
fprintf('  chord     : %f  m    \n',c)
fprintf(' ----------------------------- \n')
fprintf('  Lift(H-S) : %f N/m   \n',L)
fprintf('  Drag(H-S) : %f N/m   \n',D)
fprintf('  Lift(box) : %f N/m   \n',R(2))
fprintf('  Drag(box) : %f N/m   \n',R(1))
fprintf(' Box ------------------------- \n')
fprintf('  lx        : %f m \n',xbox(2)-xbox(1))
fprintf('  ly        : %f m \n',ybox(2)-ybox(1))
fprintf('  nx,ny     : %d , %d \n',nx,ny)
fprintf(' Uncertainty sigma^2 --------- \n')
fprintf('  sigma2(L) : %f \n',sig2L)
fprintf('  sigma2(D) : %f \n',sig2D)
fprintf(' Relative uncertainty -------- \n')
fprintf('  sigma(L)/L: %f \n',rel_err_L)
fprintf('  sigma(D)/D: %f \n',rel_err_D)
