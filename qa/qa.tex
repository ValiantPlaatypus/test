\documentclass[11pt,fleqn]{article}

\usepackage[T1]{fontenc}
\usepackage[utf8]{inputenc}
\usepackage[italian]{babel}

\usepackage{graphicx}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{bm}
\usepackage[colorlinks]{hyperref}
\usepackage{overpic}

\newcommand{\ul}[1]{\underline{#1}}
\newcommand{\uul}[1]{\ul{\ul{#1}}}
\newcommand{\p}[2]{\dfrac{\partial{#1}}{\partial{#2}}}
\newcommand{\f}[2]{\dfrac{#1}{#2}}
\newcommand{\bh}[1]{\bm{\hat{#1}}}
\newcommand{\remark}{}  % Dummy definition
\newcommand{\vel}{\bm{u}}
\newcommand{\avg}[1]{\overline{#1}}
\newcommand{\va}{\avg{\vel}}
\newcommand{\vp}{\vel'}
\newcommand{\pa}{\avg{p}}
\newcommand{\pp}{p'}
\newcommand{\grad}{\bm{\nabla}}
\newcommand{\dive}{\grad \cdot}

\title{Alcuni chiarimenti su argomenti vari ed eventuali}

\begin{document}

\maketitle

\tableofcontents


\section{Traslazione, rotazione e deformazione e deformazione di un elemento di fluido}
\input{./traslRotDef.tex}
\clearpage \newpage

\section{Legame tra le linee di corrente e le linee di livello della funzione di corrente $\psi$: problema 2D}
\input{./psi_streamlines.tex}
\clearpage \newpage

\section{Tensore degli sforzi}
\input{./stressTensor.tex}
\clearpage \newpage

\section{Equazione della quantità di moto e curvatura delle linee di corrente}
\input{./../template/capitoli/04_bilanci/0410momentum.tex}
\clearpage \newpage

\section{Teoremi di Bernoulli ed equazione della vorticità}
\input{./../template/capitoli/05_bernoulli/05teoria.tex}
\clearpage \newpage

\section{Teorema di Kutta--Jukowski}
\input{./kj.tex}
\clearpage \newpage

\section{Cenni sulla similitudine}
\input{./sim.tex}
\clearpage \newpage

\section{Equazione integrale di Von Karman per lo strato limite}
\input{./../template/capitoli/09_bl/0903_vkie.tex}

\subsection{Metodo di Thwaites per lo strato limite laminare}
\input{./../template/capitoli/09_bl/09_thwaites.tex}
\clearpage \newpage

\section{Introduzione alla turbolenza}
\input{./turbulence.tex}


\end{document}
