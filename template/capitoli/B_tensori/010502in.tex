...
Come già visto in precedenza, il tensore metrico risulta utile nel passaggio dalla componenti contravarianti a quelle 
 covarianti e viceversa, consentendo di esprimere un vettore della base $\{ \bm{b}_k \}$ nella base $\{ \bm{b}^k \}$ e viceversa tramite le (\ref{eqn:b2bd})
 \begin{equation}
...
%% $\bm{\nabla}$\rule{.4em}{.4pt}
% \paragraph{Operatore $\bm{\nabla}$(\_).} Si definisce l'operatore $\bm{\nabla}$(\_), applicando per esempio il vettore formale $\bm{\nabla}$\footnote{
% \'E possibile pensare al vettore formale $\bm{\nabla}$ come il vettore che scritto in una base contravariante ha come componenti gli
% operatori di derivata covariante $\nabla_k$, appena incontrati nella scrittura del gradiente in componenti.
% \begin{equation}
%  \bm{\nabla} = \nabla_k \bm{b}^k
% \end{equation}
% Non è la prima volta che incontrate un vettore formale: pensate al determinante formale per il calcolo del rotore, in coordinate cartesiane o
% in coordinate curvilinee ortogonali.
% } a un tensore 
% del secondo ordine $\bm{S}$
% \begin{equation}
%   \bm{\nabla}\bm{S} = \bm{\nabla} \otimes \bm{S} = (\nabla_k \bm{b}^k) \otimes (S^{ij} \bm{b}_i \otimes \bm{j}) = 
%     \nabla_k S^{ij} \bm{b}^k \otimes  \bm{b}_i \otimes \bm{b}_j
% \end{equation}
% esprimibile in componenti come (con $\nabla_k$ è stata indicata la derivata covariante, la stessa presente nel gradiente, in generale
% diversa dalla derivata parziale)
% \begin{equation}
...
% \end{equation}
% Questa operazione è differente dal gradiente del tensore $\bm{S}$.
 
% \noindent
% \paragraph{Derivata covariante.} Definita in precedenza nel paragrafo sul gradiente. Un campo tensoriale 
% è uniforme in una direzione se è nulla la derivata covariante rispetto alla relativa coordinata.

% \dots Un campo tensoriale 
% è uniforme in una direzione se è nulla la derivata covariante rispetto alla relativa coordinata \dots
% \noindent
% Verrà messo un esempio sul sito.
...
% Nello studio della meccanica dei continui viene spesso utilizzato un operatore simile ma non equivalente alla divergenza.
...
Calcolare l'espressione in componenti dell'operatore $\bm{\nabla} \cdot \underline{\hspace{8pt}}$ applicato a un campo vettoriale e un campo tensoriale del secondo ordine, in una base naturale generica. Dimostrare che coincide con la divergenza quando applicato a un campo vettoriale. Dimostrare che quando viene applicato a un campo tensoriale del secondo ordine ha l'espressione 
 ...
 \end{equation}
\end{exercise}
% Il risultato di questo operatore applicato a un tensore del secondo ordine $\bm{S}$ può quindi essere scritto nella base naturale come
...
% % -----
% Analogamente con quanto fatto per l'operatore $\bm{\nabla} \cdot$, è possibile definire l'operatore $\bm{\nabla}$ (controparte dell'operatore gradiente), tramite la sua azione su un campo tensoriale. In particolare può essere pensato come il prodotto tensoriale tra il ``vettore formale'' $\bm{\nabla} = \nabla_k \bm{b}^k$ e il tensore $\bm{T}$ al quale è applicato
% \begin{equation}
%  \bm{\nabla} \bm{T} = (\nabla_k \bm{b}^k) \ot \bm{T} .
% \end{equation}
% Per esempio, per un tensore del secondo ordine $\bm{S} = S^{ij} \bm{b}_i \ot \bm{b}_j$
% \begin{equation}
%   \bm{\nabla} \bm{S} = \bm{\nabla} \ot \bm{S} = (\nabla_k \bm{b}^k) \ot ( S^{ij} \bm{b}_i \ot \bm{b}_j ) = \nabla_k S^{ij} \bm{b}^k \ot \bm{b}_i \ot \bm{b}_j .
% \end{equation}
% Nel caso di gradiente di un campo scalare, l'operatore $\bm{\nabla}$ così definito e l'operatore gradiente danno come risultato lo stesso campo vettoriale.
% 
% %%%%%%
%%%% Conviene prestare attenzione alla distinzione tra la definizione di divergenza e dell'operatore $\bm{\nabla} \cdot (\_)$\footnote{
%%%%  La relazione di Cauchy sugli sforzi $\bm{t} = \bm{\hat{n}} \cdot \bm{T}$, in componenti $t_i = n_j T_{ji}$ (qui senza prestare
%%%%  attenzione alla posizione degli indici), lega un versore $\bm{\hat{n}}$,
%%%%  il tensore degli sforzi $\bm{T}$ e il vettore sforzo $\bm{t}$. Nel procedimento che permette di ricavare le equazioni di bilancio
%%%%  in forma differenziale da quelle in forma integrale, il termine $\oint_S \bm{t}$ che compare nell'equazione della quantità di moto,
%%%%  viene prima trasformato usando la relazione di Cauchy e poi, utilizzando una conseguenza del Lemma di Green, in un integrale di volume
%%%%  \begin{equation}
%%%%  \begin{aligned}
%%%%   \oint_S \bm{t} & = \oint_S \bm{n} \cdot \bm{T} = \\
%%%%                  & = \int_V \bm{\nabla} \cdot \bm{T}
%%%%  \end{aligned}
%%%%  \qquad \qquad
%%%%  \begin{aligned}
%%%%   \oint_S t_i & = \oint_S n_j T_{ji} = \\
%%%%               & = \int_V T_{ji/j}
%%%%  \end{aligned}
%%%%  \end{equation}
%%%% }
%%%% quando applicati
%%%% a tensori di ordine maggiore o uguale a 2. La contrazione nell'operatore $\bm{\nabla} \cdot (\_)$ avviene tra il primo e l'ultimo indice del
%%%% gradiente\footnote{
%%%% Si può pensare all'operazione $\bm{\nabla} \cdot \bm{T}$ come l'azione del ``vettore formale'' nabla sul tensore $\bm{T}$. Per esempio
%%%% su un tensore $\bm{T}$ del secondo ordine
%%%% \begin{equation}
%%%%  \nabla_i \bm{b}^i \cdot \bm{T} = \nabla_i \bm{b}^i \cdot T^{jk} \bm{b}_j \otimes \bm{b}_k = \nabla_i T^{ik} \bm{b}_k
%%%% \end{equation}
%%%% dove si è usato $\bm{b}^i \cdot \bm{b}_j = \delta_j^i$ e $\nabla_i$ indica la derivata covariante rispetto alla coordinata $q^i$.
%%%% }.
%%%% 
%%%% \paragraph{Operatore $\nabla \cdot \bm{S}$.}
%%%% Nel caso di tensore del secondo ordine è possibile definire l'operatore $\nabla \cdot \bm{S}$ partendo dall'identità
%%%% \begin{equation}
%%%%   \bm{\nabla} \cdot ( \bm{S} \cdot \bm{c} ) = (\bm{\nabla} \cdot \bm{S} ) \cdot \bm{c} + \bm{S}:\bm{\nabla}\bm{c}
%%%% \end{equation}
%%%% per ogni campo vettoriale $\bm{c}$ e per ogni campo tensoriale $\bm{S}$.
%%%%% \footnote{
%%%%% Si può definire l'operatore $\nabla \cdot \bm{T}$ applicato a un vettore del secondo ordine a partire dalla relazione
%%%%% \begin{equation}
%%%%%  \nabla \cdot (\bm{S} \cdot \bm{c}) = (\bm{\nabla} \cdot \bm{S}) \cdot \bm{c}
%%%%% \end{equation}
%%%%% per ogni campo vettoriale uniforme $\bm{c}$. Si ricorda che un campo vettoriale è uniforme, cioè costante in spazio, quando
%%%%% è nulla la sua derivata covariante.}
%%%% \noindent
%%%% La divergenza di un campo vettoriale $\bm{v}$ coincide con $\bm{\nabla} \cdot \bm{v}$ può essere scritta in componenti come
%%%% \begin{equation}
%%%%  \bm{\nabla} \cdot \bm{v} = \dfrac{\partial v^i}{\partial q^i} + \Gamma_{li}^i v^l
%%%% \end{equation}
%%%% L'operatore $\bm{\nabla} \cdot$ applicato a campo tensoriale del secondo ordine può essere scritta in componenti (contravarianti)
%%%% come\footnote{La contrazione è intesa
%%%% tra il primo indice (contravariante) e l'ultimo (covariante).  Si ricorda che l'operazione di contrazione deve avvenire tra
%%%% indici di carattere differente. Si può pensare a $\bm{\nabla} \cdot \bm{S}$ come risultato del prodotto ``dot'' tra il vettore simbolico 
%%%% $\bm{\nabla} = \nabla_k \bm{b}^k$ (è espresso nella base contravariante; le componenti covarianti sono le derivate covarianti rispetto
%%%% alle coordinate $q^i$) con il tensore $\bm{S}$
%%%% \begin{equation}
%%%% \begin{aligned}
%%%%  \bm{\nabla} \cdot \bm{S} & = (\nabla_k \bm{b}^k) \cdot (S^{ij} \bm{b}_i \otimes \bm{b}_j) = \\
%%%%   & = \nabla_i S^{ij} \bm{b_j} =  \\
%%%%   & = \left[ \dfrac{\partial S^{ij}}{\partial q^i} + \Gamma_{il}^i S^{lj} + \Gamma_{il}^j S^{il} \right] \bm{b_j}
%%%% \end{aligned}
%%%% \end{equation}
%%%% }
%%%%%Questa operazione è immediata se si prende la forma in componenti di $\bm{\nabla} \bm{S}$ riportata
%%%%% in precedenza
%%%%% \begin{equation}
%%%%%   \bm{\nabla} \bm{S}  = \left[ \dfrac{\partial S^{ij}}{\partial q^k} + \Gamma_{kl}^i S^{lj} + \Gamma_{kl}^j S^{il} \right]
%%%%%             \bm{b}_i \otimes \bm{b}_j \otimes \bm{b}^k
...
%%%%  \bm{\nabla} \cdot \bm{S} = \left[ \dfrac{\partial S^{ij}}{\partial q^i} + \Gamma_{il}^i S^{lj} + \Gamma_{il}^j S^{il} \right] \bm{b}_j
%%%% \end{equation}
%%%% Si noti la differenza con la divergenza di un tensore del secondo ordine. Questo è l'operatore che agisce sul tensore
%%%% degli sforzi nella forma differenziale dell'equazione della quantità di moto per i mezzi continui.\footnote{
%%%% Per continui non polari, il tensore degli sforzi $\bm{T}$ è un tensore del secondo ordine simmetrico $T^{ij}=T^{ji}$.
%%%% Una volta che si è capita la differenza tra $\bm{\nabla} \cdot \bm{T}$ e $\text{div}\bm{T}$, per tensori simmetrici del secondo ordine
%%%% le due operazioni portano allo stesso risultato. L'imprecisione in questa miriade di indici,
%%%% ha non ha nessun effetto rilevante.
%%%% }.
%%%% 
%%%% \noindent
%%%% Si noti infine come la divergenza e l'operatore $\bm{\nabla} \cdot$ riducono di 1 l'ordine del tensore (se $\bm{S}$ è del secondo ordine,
%%%% $\bm{\nabla} \bm{S}$ del terzo ordine, $\bm{\nabla} \cdot \bm{S}$ del primo).%, sommando sulle componenti $i$ e $k$
%%%%  
%%%%% \paragraph{Forme alternative.} \dots
%%%% 
...
% \noindent 
% Quando $\bm{u}$, $\bm{v}$ sono vettori, il termine advettivo $(\bm{u}\cdot\bm{\nabla})\bm{v}$
% può essere scritto anche come
% \begin{equation}
%  (\bm{u} \cdot \bm{\nabla}) \bm{v} = \text{grad}\bm{v} \cdot \bm{u}
% \end{equation}
% o può essere ricavato dall'identità
% \begin{equation}
%  (\bm{u} \cdot \bm{\nabla}) \bm{v} = \bm{\nabla} \cdot (\bm{u} \otimes \bm{v}) - (\bm{\nabla} \cdot \bm{u}) \bm{v}
% \end{equation}
% \noindent
% In componenti contravarianti
% \begin{equation}
%  (\bm{u} \cdot \bm{\nabla}) \bm{v} = \left[ u^i \left(\dfrac{\partial v^j}{\partial q^i} + \Gamma_{il}^j v^l \right)  \right] \bm{b}_j
% \end{equation}
 
 
 
% \paragraph{Alcune osservazioni.} Le definizioni degli operatori date nei paragrafi precedenti possono risultare astratte, in quanto non è mai stato fatto riferimento ai teoremi integrali (Stokes, divergenza, rotore) nei quali compaiono e dai quali si può ``intuirne il significato fisico''. La forma in componenti degli operatori a partire dai teoremi integrali verrà ricavata in un paragrafo dedicato esclusivamente ai sistemi di coordinate curvilinee ortogonali.
