% compute the velocity induced by a unitary linear vortex elem_j on the point rr_i
% Inputs :
...
% Outputs:
% - v(2,1): velocity induced in the point rr_i by the vortex distribution on elem_j
% 

function v = compute_velocity_vortex( elem_j , rr_i )
...
% v = -1/(2*pi)*bij     * elems_j.tver + ...
%     -1/(2*pi)*ln(sij) * elems_j.nver ,
% where:
% - sij is the ratio |r2|/|r1|
sij = norm(r2)/norm(r1) ; % ratio of their norm
...
vstar = [ -bij /(2*pi) ; -log(sij)/(2*pi) ] ; % vstar = [ vt ; vn ]
...
% v = vx * \hat{x} + vy * \hat{y} ;
v = [ elem_j.tver , elem_j.nver ] * vstar ; % v = [ vx ; vy ]
